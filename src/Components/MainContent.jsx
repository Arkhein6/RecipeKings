...
const MainContent = () => {
    ...
    const [BlogSectionCards,setBLogSectionCards] = React.useState(blogSectionImages)
    ...
    const BlogSectionElements = blogSectionImages.map((curr,idx) => {
        return <BlogCard key={idx} image={curr.image} description='Lorem ipsum dolor sit amet consecterur adipiscing elit somredthe' date={curr.date}title='Our Recipes Blog'/>
    })
    const FeaturedRecipeElements =categoryRecipes.map((curr) => {
        return <RecipeCard key={curr.Id} featured Name={curr.Category} Image={curr.Image} 
        /> })
